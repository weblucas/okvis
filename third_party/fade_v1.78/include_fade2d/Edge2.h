...
class CLASS_DECLSPEC Edge2 {
 public:
   ...
   * @param oppIdx_ is intra-triangle-index of the opposite vertex (of the edge)
   * in pT
   *
   ...
   *
   * \param pT0 is used to return the triangle whose directed edge the present
   * edge is \param idx0 is the opposite intra-triangle-index in pT0 of the
   * present edge \param pT1 is the other adjacent triangle at the present edge
   * (or NULL) \param idx1 is the intra-triangle index of the present edge in
   * pT1 (or -1)
   *
   */
  void getTriangles(Triangle2*& pT0, Triangle2*& pT1, int& idx0,
                    int& idx1) const;
...
struct Func_ltEdge2D {
  bool operator()(const Edge2& e0, const Edge2& e1) const {
    if (e0.getLength2D() < e1.getLength2D())
      return true;
    else
      return false;
  }
...
struct Func_ltEdge25D {
  bool operator()(const Edge2& e0, const Edge2& e1) const {
    if (e0.getLength25D() < e1.getLength25D())
      return true;
    else
      return false;
  }
