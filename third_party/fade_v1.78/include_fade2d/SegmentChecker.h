...
/** The Segment intersection type enumerates the way two line segments intersect
 * each other
 *
 */
enum SegmentIntersectionType {
  ...
  SIT_SEGMENT, /**< The intersection is a non-degenerate segment (collinear
                  intersection) */
  SIT_POINT, /**< The intersection is a single point differnt from the endpoints
              */
  SIT_ENDPOINT /**< The two segments share a common endpoint which is the only
                  intersection */
};
 ...
 *
 * \image html identify_intersecting_segments.png "Polylines: Intersecting
 * segments are automatically found" \image latex
 * identify_intersecting_segments.eps "Polylines: Intersecting segments are
 * automatically found" width=15cm
 *
...
class CLASS_DECLSPEC SegmentChecker {
 public:
   ...
   *
   * \image html show_line_segments.png "Line segments written to a postscript
   * file" \image latex show_line_segments.eps "Line segments written to a
   * postscript file"  width=15cm
   */
  void showSegments(const std::string& name) const;

  /** Write a postscript file, highlight illegal segments
   *
   * @param bAlsoEndPointIntersections specifies if intersections at endpoints
   * are also illegal
   * @param name is the output filename
   *
   * \image html show_illegal_segments_count.png "Visualization of polyline
   * intersections" \image latex show_illegal_segments_count.eps "Visualization
   * of polyline intersections"  width=15cm
   */
  void showIllegalSegments(bool bAlsoEndPointIntersections,
   ...
   *
   * \image html segment_intersection_types.png "Segment intersections: (1)
   * Non-collinear, (2) collinear, (3) duplicate segments, (4) endpoint
   * intersection"
   *
   * @param bAlsoEndPointIntersections specifies if intersections at endpoints
   * shall be detected
   ...
   *
   */
  void getIllegalSegments(bool bAlsoEndPointIntersections,
                          std::vector<Segment2*>& vIllegalSegmentsOut) const;

  /** Get the intersection type of two segments
   *
   * @param pSeg1,pSeg2 are the segments to be checked
   *
   * @return SIT_NONE (no intersection),\n SIT_SEGMENT (collinear
   * intersection),\n SIT_POINT (intersection somewhere between the endpoints)
   ...
   */
  SegmentIntersectionType getIntersectionType(const Segment2* pSeg1,
                                              const Segment2* pSeg2) const;

  /** Return segments that intersect a certain segment along with their
   * intersection type
   *
   ...
   */
  void getIntersectors(
      Segment2* pTestSegment, bool bAlsoEndPointIntersections,
      std::vector<std::pair<Segment2*, SegmentIntersectionType> >&
          vIntersectorsOut) const;
   ...
   *
   * @param sit is the segment intersection type (SIT_POINT or SIT_ENDPOINT for
   * the present method)
   ...
   */
  void getIntersectionPoint(SegmentIntersectionType sit, const Segment2& seg0,
                            const Segment2& seg1, Point2& ispOut0,
                            Point2& ispOut1) const;
  /** Compute the intersection segment(s) of two collinear intersecting segments
   *
   * @param seg0,seg1 are intersecting segments such that their
   SegmentIntersectionType is SIT_SEGMENT
  ...
  void getIntersectionSegment(const Segment2& seg0, const Segment2& seg1,
                              Segment2& issOut0, Segment2& issOut1) const;
#else
   ...
   * @param typ is the intersection type (SIT_POINT or SIT_ENDPOINT for the
   present method)
   * @param seg0,seg1 are the intersecting segments
   ...
   *
  */
  void getIntersectionPoint(SegmentIntersectionType typ, const Segment2& seg0,
                            const Segment2& seg1, Point2& ispOut) const;
  ...
  /** Return the intersection type as a human readable string. This is a
  ...
  */
  SegmentIntersectionType getIntersectionType_degeneratePart(
      const Segment2*& pSeg1, const Segment2*& pSeg2, Point2& p1, Point2& p2,
      Point2& p3, Point2& p4) const;
