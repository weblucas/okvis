...
inline int inc1(int num) {
  ++num;
  if (num > 2) return 0;
  return num;
}

inline int inc2(int num) {
  --num;
  if (num < 0) return 2;
  return num;
}
 ...
 *
 * Iterates over all triangles incident to a given vertex in a circular manner.
 * Thereby, counterclockwise is the positive direction.
 *
 * \image html umbrella.jpg "Left: the iterator visits the triangles around a
 * vertex. Right: The iterator 'jumps' over the border edges of the
 * triangulation" \image latex umbrella.eps "Left: the iterator visits the
 * triangles around a vertex. Right: The iterator 'jumps' over the border edges
 * of the triangulation" width=10cm
 *
 */
class CLASS_DECLSPEC TriangleAroundVertexIterator {
 public:
  /** \brief Constructor
   *
   * @param pPnt_ is the vertex whose incident triangles can be visited with the
   * iterator
   *
   * The iterator will start at an arbitrary triangle
   */
  explicit TriangleAroundVertexIterator(const Point2* pPnt_)
      : pPnt(pPnt_), pTr(pPnt_->getIncidentTriangle()), pSavedTr(NULL) {
    if (pTr == NULL) {
      // flog("TriangleAroundVertexIterator::TriangleAroundVertexIterator(),
      // created from an invalid point");
  ...
  TriangleAroundVertexIterator(Point2* pPnt_, Triangle2* pTr_)
      : pPnt(pPnt_), pTr(pTr_), pSavedTr(NULL) {
    assert(pTr != NULL);
  }
  ...
  TriangleAroundVertexIterator(const TriangleAroundVertexIterator& it)
      : pPnt(it.pPnt), pTr(it.pTr), pSavedTr(NULL) {
    assert(pTr != NULL);
  }

  TriangleAroundVertexIterator& operator=(
      const TriangleAroundVertexIterator& other) {
    pPnt = other.pPnt;
    pTr = other.pTr;
  ...
  TriangleAroundVertexIterator& operator++() {
    ...
    // std::cout<<"ccwIdx="<<ccwIdx<<"now swapping saved="<<pSavedTr<<",
    // pTr="<<pTr<<std::endl;;
    std::swap(pSavedTr, pTr);
    pTr = pSavedTr->getOppositeTriangle(ccwIdx);
    // std::cout<<"and pTr is now the opposite triangle of
    // pSavedTr="<<*pSavedTr<<", namely "<<pTr<<std::endl;
   ...
   *
   * @warning At the border of a triangulation, two border edges are incident to
   * the center vertex. Consequently, the neighbor triangles are NULL there. If
  ...
  /** \brief Check if the center points and the current triangles of the
  ...
  /** \brief Check if the center points or the current triangles of the
   ...
   *
   * Dereferencing the iterator yields a pointer to the triangle to which the
   * iterator points.
   * @warning This method might yield NULL at the border of a triangulation.
   */
  Triangle2* operator*() { return pTr; }
