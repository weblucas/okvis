 ...
 *
 * Each triangle has three neighbors which can be accessed through
 * intra-triangle-indices: The i-th neighbor triangle of a certain triangle T is
 * the one which shares an edge with T such that this edge does not include the
 * i-th corner of T.
 *
 * \image html neig.jpg "Indices and neighborships, \e tb is the 0-th neighbor
 * of \e ta and \e ta is the 2nd neighbor of \e tb." \image latex neig.eps
 * "Indices and neighborships, \e tb is the 0-th neighbor of \e ta and \e ta is
 * the 2nd neighbor of \e tb." width=6cm
 *
 * \see TriangleAroundVertexIterator to find out how to access all triangles
 * incident to a certain vertex.
 */
class CLASS_DECLSPEC Triangle2 {
 public:
   ...
   *
   * \image html corners.jpg "Intra triangle indices are ordered
   * counterclockwise" \image latex corners.eps "Intra triangle indices are
   * ordered counterclockwise" width=4cm
   *
   ...
   *
   * @return a std::pair<Point2,bool>, where the first component is the dual
   * Voronoi vertex of the triangle and the second component is a boolean value
   * which is true if the vertex is accurate. The z-coordinate of the returned
   * point is always 0. Use Fade_2D::getHeight(..) to determine the height
   * value.
   *
   ...
   * domain of the data points. This will automatically restrict the Voronoi
   * diagram of the data points to this range.
   *
   *
   */
   ...
   *
   * \if SECTION_FADE25D
   * Note: The getInteriorAngle() method is deprecated and replaced by
   * getInteriorAngle2D() and getInteriorAngle25D() \else Note: The getArea()
   * method is deprecated and replaced by getInteriorAngle2D() to keep the names
   * consistent. \endif
   *
   ...
   *
   * \image html getITI.jpg "Intra triangle index of a vertex pointer"
   * width=10cm \image latex getITI.eps "Intra triangle index of a vertex
   * pointer" width=4cm
   *
   ...
   * \image latex getITI_t.eps "pTriangle is the 0-th neighbor of *this"
   * width=8cm
   *
   *
   * @param pTriangle is a neighbor triangle of *this.
   *
   * @return the intra-triangle-index of the vertex in \e *this which is
   * opposite (i.e., does not touch the neighbor) \e pTriangle.
   */
  ...
  /** @private
   */
  double getSquaredEdgeLength(
      int ith) const;  // Deprecated, use getSquaredEdgeLength2D()
   ...
   * \image latex getITI_t.eps "Make pTriangle the 0-th neighbor of *this"
   * width=8cm
   *
   *
   * @param ith is the index of the corner of \e *this which does not touch \e
   * pTriangle
   * @param pTriangle is a pointer to the triangle which shares two corners with
   * \e *this
   */
   ...
   */
  void setPropertiesAndOppT(Point2* pI, Point2* pJ, Point2* pK,
                            Triangle2* pNeig0, Triangle2* pNeig1,
                            Triangle2* pNeig2);
  ...
  friend inline void registerTriangles(Triangle2* fromTriangle, int ith,
                                       Triangle2* toTriangle, int jth);
 ...
 protected:
  ...
  void getCC_strategy2(int maxIdx, double avgOffX, double avgOffY,
                       Point2& cc) const;
...
inline int Triangle2::getIntraTriangleIndex(const Point2* p0,
  ...
  std::cout << "BUG: Triangle2::getIntraTriangleIndex failed for"
            << std::endl;           // COUTOK
...
inline int Triangle2::getIntraTriangleIndex(const Triangle2* pTriangle) const {
  ...
  assert(false);
#endif
  return ((aOppTriangles[1] == pTriangle) +
          2 * (aOppTriangles[2] == pTriangle));
}
...
//{
// std::cout<<"Triangle2::getIntraTriangleIndex, pTriangle is not a neighbor of
// the current triangle"<<std::endl;// COUTOK std::cout<<"Current triangle:
// "<<*this<<std::endl;// COUTOK std::cout<<"pTriangle:
// "<<*pTriangle<<std::endl;// COUTOK
...
inline void Triangle2::setPropertiesAndOppT(Point2* pI, Point2* pJ, Point2* pK,
                                            Triangle2* pNeig0,
                                            Triangle2* pNeig1,
                                            Triangle2* pNeig2) {
  assert((pI != NULL && pJ != NULL && pK != NULL));
...
inline void registerTriangles(Triangle2* pFromT, int ith, Triangle2* pToT,
                              int jth) {
  assert(checkRange(ith));
