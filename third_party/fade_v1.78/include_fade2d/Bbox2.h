...
class CLASS_DECLSPEC Bbox2 {
 public:
  ...
  Bbox2(std::vector<Point2>::const_iterator start_it,
        std::vector<Point2>::const_iterator end_it, GeomTest* pGeomTest_ = NULL)
      : minX(DBL_MAX),
        minY(DBL_MAX),
        maxX(-DBL_MAX),
        maxY(DBL_MAX),
  ...
  bool isValid() const { return minX < DBL_MAX; }
   ...
   *
   * Two valid bounding boxes intersect if they share at least one point in the
   * XY plane.
   */
  ...
  bool add(std::vector<Point2*>::const_iterator start_it,
    ...
    if (bValid) {
      ...
      if (oldMinX != minX || oldMinY != minY || oldMaxX != maxX ||
          oldMaxY != maxY)
        return true;
      return false;
    } else {
   ...
   *
   * @return true if minX <= p.x() <=maxX and minY <= p.y() <=maxY or false
   * otherwise.
   */
 ...
 *
 * \return the corner point with the minimum coordinates, the z-coordinate is
 * set to 0
 */
#else
/** \brief Get the min point
 ...
 *
 * \return the 2D corner point with the maximum coordinates, the z-coordinate is
 * set to 0
 */
#else
/** \brief Get the max point
 ...
 */
  ...
  double getRangeX() const { return maxX - minX; }
  /** \brief Get y-range
   *
   * \return maxY-minY
   */
  double getRangeY() const { return maxY - minY; }
   ...
   *
   */
  void getBounds(double& minX_, double& maxX_, double& minY_,
                 double& maxY_) const;
...
inline std::ostream& operator<<(std::ostream& stream, Bbox2& pC) {
  stream << "Bbox2: (" << pC.minX << "," << pC.minY << ") -> (" << pC.maxX
         << "," << pC.maxY << "), rangeX=" << pC.getRangeX()
         << ", rangeY=" << pC.getRangeY();
  return stream;
}
