...
template <typename T>
inline void unusedParameter(const T&) {}  // Avoids compiler warnings
...
class CLASS_DECLSPEC MeshGenParams {
 public:
  explicit MeshGenParams(Zone2* pZone_)
    ...
    if (pZone->getZoneLocation() != ZL_INSIDE &&
      ...
      std::cerr << "\n\n\nMeshGenParams::MeshGenParams(..), ERROR: "
                   "\n\tbehavior since Fade 1.39: The zoneLocation must be "
                   "ZL_INSIDE or ZL_BOUNDED. Please convert the zone using "
                   "pZone->convertToBoundedZone(..)\n\n"
                << std::endl;
      assert(false);
      return;
    }
   ...
   * \image html mesh-generator-customParameters.png "User Controlled Mesh
   * Density, Area" \image latex mesh-generator-customParameters.eps "User
   * Controlled Mesh Density, Area" width=7cm
   */
   ...
   * \image html mesh-generator-customParameters.png "User Controlled Mesh
   * Density, Edge Length" \image latex mesh-generator-customParameters.eps
   * "User Controlled Mesh Density, Edge Length" width=7cm
   */
   ...
   * automatically subdivided. If a custom implementation of
   * getMaxTriangleArea(Triangle* pT) is provided then this value is ignored.
   ...
   * one) area(t0)/area(t1) < growFactor. Recommendation: growFactor>5.0,
   * Default: growFactor=DBL_MAX
   */
  double growFactor;
   ...
   * \image latex grid-mesh-angle.eps "Grid Meshing along Vector2(1.0,0.3,0.0)"
   * width=7cm \else \image html grid-mesh-angle.png "Grid Meshing along
   * Vector2(1.0,0.3)" \image latex grid-mesh-angle.eps "Grid Meshing along
   * Vector2(1.0,0.3)" width=7cm \endif
   */
  Vector2 gridVector;
