 ...
 * img[src="constrained_delaunay.png"]{width:40%;}</style> \endhtmlonly \image
 * html constrained_delaunay.png "Constrained Delaunay triangulation" \image
 * latex constrained_delaunay.eps "Constraint Delaunay triangulation" width=10cm
 */
class ConstraintGraph2 {
 public:
  /// @private
  CLASS_DECLSPEC
  ConstraintGraph2(Dt2* pDt2_, std::vector<ConstraintSegment2*>& vCSegments,
                   ConstraintInsertionStrategy);

  /// @private
  CLASS_DECLSPEC
  ConstraintGraph2(Dt2* pDt2_, std::vector<ConstraintSegment2*>& vCSegments_,
   ...
   * if the ConstraintGraph2 has been created with
   * Fade_2D::createConstraint(..,..,\p bOrientedSegments=true).
   *
   ...
   *
   * \if SECTION_FADE25D
   * @return CIS_CONFORMING_DELAUNAY, CIS_CONFORMING_DELAUNAY_SEGMENT_LEVEL or
   * @n CIS_CONSTRAINED_DELAUNAY \else
   ...
   */
  CLASS_DECLSPEC
  void getOriginalConstraintSegments(
   ...
   */
  CLASS_DECLSPEC
  void getChildConstraintSegments(
      std::vector<ConstraintSegment2*>& vConstraintSegments_) const;
  ...
  /// @private
  void updateSplittedConstraintSegment(ConstraintSegment2* pCSeg,
                                       bool bUpdateCMGR);
  /**
   ...
   */
  CLASS_DECLSPEC
  void getDirectChildren(ConstraintSegment2* pParent,
                         ConstraintSegment2*& pChild0,
                         ConstraintSegment2*& pChild1);
  /// @private
  void getAliveConstraintChain(
      std::vector<ConstraintSegment2*>& vAliveCSeg);  // For debugging
  /// @private
 ...
 protected:
  ...
  bool bIsPolygon;
  std::map<ConstraintSegment2*, bool, func_ltDerefPtr<ConstraintSegment2*> >
      mCSegReverse;
