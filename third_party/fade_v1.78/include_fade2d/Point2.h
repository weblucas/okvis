...
/** \brief Point
 *
 * This class represents a point in 2D with x- and y-coordinates and an
 * additional pointer to an associated triangle.
 */
class Point2 {
#ifdef GEOM_DBG
  int getSeqNum() {
 ...
 public:
   ...
   *
   * Create a point as a copy of p_. The associated triangle pointer is
   * initialized to NULL
   */
  CLASS_DECLSPEC
 ...
 */
...
#else
  void print() { std::cout << coordX << " " << coordY << std::endl; }
#endif

  // Deprecated and undocumented, use setHeight() instead. Kept for backward
  // compatibility
  /** @private
   *
   */
  CLASS_DECLSPEC
  void setZ(double z) { setHeight(z); }
  ...
  Point2(const double x_, const double y_)
      : coordX(x_), coordY(y_), pAssociatedTriangle(NULL), customIndex(-1) {
   ...
   */
  CLASS_DECLSPEC
  double x() const { return coordX; }
  /** \brief Get the y-coordinate
   *
   * @return the y-coordinate
   */
  CLASS_DECLSPEC
  double y() const { return coordY; }
   ...
   */
  CLASS_DECLSPEC
  void setHeight(double z) { coordZ = z; }
#endif
   ...
   *
   * @note Although a point has a z-coordinate in the 2.5D version only x and y
   * a compared by this method
   */
  CLASS_DECLSPEC
   ...
   */
  CLASS_DECLSPEC
  Triangle2* getIncidentTriangle() const { return pAssociatedTriangle; }
 ...
 */
   ...
   *
   * \see A best practices example that deals with indices:
   * http://www.geom.at/runtime/
   *
   */
  CLASS_DECLSPEC
  void setCustomIndex(int customIndex_) { customIndex = customIndex_; }
   ...
   */
  CLASS_DECLSPEC
  int getCustomIndex() const { return customIndex; }

  /** \brief Associate a triangle with the point
   *
   * @param pT will be associated with the triangle
   */
  void setIncidentTriangle(Triangle2* pT) { pAssociatedTriangle = pT; }
...
#if GEOM_PSEUDO3D == GEOM_TRUE
  stream << "Point2 (" << &pnt << "," << pnt.customIndex << "): " << pnt.x()
         << ", " << pnt.y() << ", " << pnt.z();
  //#ifndef NDEBUG
 ...
 */
inline double sqDistance2D(const Point2& p0, const Point2& p1) {
 ...
 * @note This method will not use the z-coordinate (if any)
 */
inline double sqDistance2D(const double x0, const double y0, const Point2& p1) {
 ...
 *
 * @note In contrast to sqDistance2D this method will use the z-coordinate of
 * the points
 */
inline double sqDistance25D(const Point2& p0, const Point2& p1) {
  double deltaX = p1.x() - p0.x();
  double deltaY = p1.y() - p0.y();
 ...
 * the points
 */
inline double sqDistance25D(const double x0, const double y0, const double z0,
                            const Point2& p1) {
  double deltaX = p1.x() - x0;
  double deltaY = p1.y() - y0;
...
inline Point2 center(const Point2& p0, const Point2& p1) {
#if GEOM_PSEUDO3D == GEOM_TRUE
