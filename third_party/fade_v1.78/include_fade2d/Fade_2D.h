 ...
 *
 * Fade_2D represents a Delaunay triangulation in 2D or 2.5D (depends on the
 * used namespace)
 */
class CLASS_DECLSPEC Fade_2D {
 public:
   ...
   */
  bool checkValidity(bool bCheckEmptyCircleProperty,
   ...
   * @param bWithConstraints specifies if constraint segments shall be shown
   * (default: true)
   */
  void show(const std::string& postscriptFilename,
            bool bWithConstraints = true) const;

  /** \brief Draws the triangulation as postscript file using an existing
   * Visualizer2 object
   *
   ...
   *
   * @param pVis is the pointer of a Visualizer2 object that may already contain
  ...
  void getConvexHull(bool bAllVertices,
   ...
   *
   * The triangulation keeps a copy of \e p. The return value is a pointer to
   * this copy. If duplicate points are inserted, the triangulation does not
   * create new copies but returns a pointer to the copy of the very first
   * insertion. @n
   * @n
   * @note This method offers a very good performance but it is still faster if
   * all points are passed at once, if possible.
   */
 ...
 * @param aCoordinates is an array of \e 3n double values, e.g.
 * {x0,y0,z0,x1,y1,z1,...,xn,yn,zn}
 * @param aHandles is an array with size \e n where pointers to the inserted
 ...
 *
 * @param numPoints is the number of points to be inserted
 * @param aCoordinates is an array of \e 2n double values, e.g.
 * {x0,y0,x1,y1,...,xn,yn}
 * @param aHandles is an empty array with size \e n where pointers to the
 * inserted points will be stored by Fade
 *
 * \note Use Fade_2D::setNumCPU() to activate multithreading
 */
#endif
   ...
   * @return a pointer to a Triangle2 object (or NULL if is2D()==false or if \e
  ...
  bool getHeight(double x, double y, double& heightOut,
                 Triangle2* pApproxT = NULL, double tolerance = 0) const;
#endif
   ...
   *
   * @param pZone is the zone whose triangles are refined. Allowed zoneLocation
   * values are ZL_INSIDE and ZL_BOUNDED.
   * @param minAngleDegree (up to 30) is the minimum interior triangle angle
   * @param minEdgeLength is a lower threshold on the edge length. Triangles
   * with smaller edges are not refined.
   * @param maxEdgeLength is an upper threshold on the edge length. Triangles
   * with larger edges are always refined.
   * @param bAllowConstraintSplitting specifies if constraint edges may be
   * splitted
   *
   ...
   * convert other types of zones to ZL_BOUNDED using @ref convertToBoundedZone
   * "Zone2::convertToBoundedZone()".
   */

  void refine(Zone2* pZone, double minAngleDegree, double minEdgeLength,
              double maxEdgeLength, bool bAllowConstraintSplitting);
  ...
  <a href="http://www.geom.at/wp/wp-content/uploads/delaunay_triangulation.jpg">
   <img style="width: 95%"
  src="http://localhost/geom_homepage_www/fade25d/html/mesh-generator-customParameters.png"
  alt="Control over the local mesh density in a Delaunay Mesh" /></a> <p>Control
  over the local mesh density</p>
  </div>
  <div class="sbs4image">
  <a href="http://www.geom.at/wp/wp-content/uploads/constrained_delaunay.jpg">
   <img style="width: 95%"
  src="http://www.geom.at/wp-content/uploads/mesh-generator-maxLength.png"
  alt="Delaunay meshing with maximum edge length" /></a> <p>Delaunay meshing
  with maximum edge length</p>
  </div>
  <div class="sbs4image">
  <a href="http://www.geom.at/wp/wp-content/uploads/delaunay_zones.png">
   <img style="width: 95%"
  src="http://localhost/geom_homepage_www/fade25d/html/grid-mesh.png"
  alt="Axis-aligned grid mesh" /></a> <p>Grid mesh, axis-aligned</p>
  </div>
  <div class="sbs4image">
  <a href="http://www.geom.at/wp/wp-content/uploads/delaunay-mesher-fade_l.png">
   <img style="width: 95%"
  src="http://localhost/geom_homepage_www/fade25d/html/grid-mesh-angle.png"
  alt="Grid mesh aligned to a certain direction" /></a> <p>Grid mesh, aligned to
  ...
  */
   ...
   *
   * @return the number of triangles in the triangulation (or 0 as long as
   * is2D() is false).
   *
   */
   ...
   *
   * Stores pointers to all vertices of the triangulation in vAllPoints. The
   * order in which the points are stored is \e not necessarily the insertion
   * order. For geometrically identical points which have been inserted multiple
   * times, only one pointer exists. Thus vAllPoints.size() can be smaller than
   ...
   *
   * @return the triangle that has the edge (p0,p1) or NULL if no such edge is
   * present
   *
   ...
   *
   * As long as all inserted points are collinear the triangulation does not
   * contain triangles. This is clearly the case as long as less than three
   * input points are present but it may also be the case when 3 or more points
   * have been inserted when all these points are collinear. These points are
   * then in a pending state, i.e. they will be triangulated as soon as the
   * first non-collinear point is inserted.
   *
   * \image html coll.jpg "Triangles are generated as soon as the first
   * non-collinear point is inserted." \image latex coll.eps "Triangles are
   * generated as soon as the first non-collinear point is inserted." width=7cm
   *
  ...
  *
  * \htmlonly <style>div.image img[src="bare-delaunay.png"]{width:40%;}</style>
  \endhtmlonly
  * \image html bare-delaunay.png "Delaunay triangulation without constraints"
  * \image latex bare-delaunay.eps "Delaunay triangulation without constraints"
  width=6cm

  * \htmlonly <style>div.image
  img[src="constrained_delaunay.png"]{width:40%;}</style> \endhtmlonly
  * \image html constrained_delaunay.png "Constrained Delaunay triangulation"
  * \image latex constrained_delaunay.eps "Constraint Delaunay triangulation"
  width=6cm
  *
  * \htmlonly <style>div.image
  img[src="conforming-delaunay-makeDelaunay.png"]{width:40%;}</style>
  \endhtmlonly
  * \image html conforming-delaunay-makeDelaunay.png "Conforming Delaunay
  triangulation through the ConstraintGraph::makeDelaunay() method"
  * \image latex conforming-delaunay-makeDelaunay.png "Conforming Delaunay
  triangulation through the ConstraintGraph::makeDelaunay() method" width=6cm
  * \htmlonly </div> \endhtmlonly
  *
  */
  ConstraintGraph2* createConstraint(std::vector<Segment2>& vSegments,
   ...
   * zoneLoc==ZL_GLOBAL.
   * @param bVerbose is by default true and causes a warning if NULL is
   * returned.
   *
  ...
  /** \brief Create a zone limited by multiple ConstraintGraph2 objects by
   * growing from a start point
   *
   ...
   * @param bVerbose is by default true and causes a warning if NULL is
   * returned.
   * @return a pointer to the new Zone2 object (or NULL if zoneLoc!=ZL_GROW or
   * no triangles exist)
   */
  Zone2* createZone(const std::vector<ConstraintGraph2*>& vConstraintGraphs,
                    ZoneLocation zoneLoc, const Point2& startPoint,
                    bool bVerbose = true);

  /** \brief Create a zone limited by a ConstraintGraph by growing from a start
   * point
   *
   * A Zone2 object is an area of the traingulation, see \ref createZone
   *
   * @param pConstraintGraph is a constraint whose edges specify the area's
   * border
   * @param zoneLoc must be ZL_GROW
   * @param startPoint is the point from which the area is grown until the
   * borders specified in pConstraint are reached
   * @param bVerbose is by default true and causes a warning if NULL is
   * returned.
   * @return a pointer to the new Zone2 object (or NULL if zoneLoc!=ZL_GROW or
   * no triangles exist)
  ...
  Zone2* createZone(ConstraintGraph2* pConstraintGraph, ZoneLocation zoneLoc,
                    const Point2& startPoint, bool bVerbose = true);
   ...
   */
  void getAliveConstraintSegments(
      std::vector<ConstraintSegment2*>& vAliveConstraintSegments) const;

  /** \brief Get all (alive and dead) constraint segments
   */
  void getAliveAndDeadConstraintSegments(
      std::vector<ConstraintSegment2*>& vAllConstraintSegments) const;
   ...
   */
  void getIncidentTriangles(Point2* pVtx,
   ...
   */
  void getIncidentVertices(Point2* pVtx,
   ...
   *
   * @param msgType is the type of message the subscriber shall receive, e.g.
   * MSG_PROGRESS or MSG_WARNING
   ...
   *
   * @param msgType is the type of message the subscriber shall not receive
   * anymore
   * @param pMsg is a pointer to a custom class derived from MsgBase
   */
   ...
   */
  Zone2* importTriangles(std::vector<Point2>& vPoints, bool bReorientIfNeeded,
                         bool bCreateExtendedBoundingBox);

  /** \brief Compute the orientation of 3 points
   *
   * \return ORIENTATION2_COLLINEAR, ORIENTATION2_CW (clockwise) or
   * ORENTATION2_CCW (counterclockwise)
   */
  Orientation2 getOrientation(const Point2& p0, const Point2& p1,
                              const Point2& p2);
   ...
   * void cutTriangles(std::vector<Segment2>& vSegments,bool
   * bTurnEdgesIntoConstraints) should be used. The reason is that each
   * individual cut operation changes the triangulation and thus iterative calls
   * to the present version of the method can lead to a different result. \n
   *
   *
   */
  void cutTriangles(const Point2& knifeStart, const Point2& knifeEnd,
                    bool bTurnEdgesIntoConstraints);

  /** \brief Cut through a triangulation
   *
   * \param vSegments are the knife segments
   * \param bTurnEdgesIntoConstraints specifies if intersected edges shall
   * automatically be turned into constraints
   *
   * Same method as void cutTriangles(const Point2& knifeStart,const Point2&
   * knifeEnd,bool bTurnEdgesIntoConstraints) but it takes a vector of segments
   * instead of a single segment. This is the recommended method to cut through
   * a triangulation when more than one knife segment exists.
   *
   */
 ...
 *
 * \if SECTION_FADE25D
 * \htmlonly <style>div.image img[src="drape.png"]{width:100%;}</style>
 * \endhtmlonly
 * * \image html drape.png "Drape: Input segments (blue) are draped (red) onto a
 * TIN. Left with tolerance 1.0, right without tolerance" \image latex drape.eps
 * "Drape: Input segments (blue) are draped (red) onto a TIN. Left with
 * tolerance 1.0, right without tolerance" width=10cm \else \htmlonly
 * <style>div.image img[src="drape2d.png"]{width:70%;}</style> \endhtmlonly
 * * \image html drape2d.png "Drape: Input segments are draped (red) onto a TIN.
 * They are subdivided (blue) at intersections with triangulation edges" \image
 * latex drape2d.eps "Drape: Input segments are draped (red) onto a TIN. They
 * are subdivided (blue) at intersections with triangulation edges" width=6cm
 * \endif
 *
 ...
 */
  ...
  bool drape(std::vector<Segment2>& vSegmentsIn,
             std::vector<Segment2>& vSegmentsOut, double zTolerance) const;
#else
  bool drape(std::vector<Segment2>& vSegmentsIn,
 ...
 */
CLASS_DECLSPEC
bool reconstruct(std::vector<Point2>& vInPoints,
