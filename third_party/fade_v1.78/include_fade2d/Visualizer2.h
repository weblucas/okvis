...
class Visualizer2 {
 public:
   ...
   */
  CLASS_DECLSPEC
  void addObject(const std::vector<ConstraintSegment2*>& vConstraintSegments,
                 const Color& c);
   ...
   *
   * @note This method \e must be called at the end when all the objects have
   * been added.
   */
  CLASS_DECLSPEC
  void writeFile();
 ...
 protected:
  ...
  void writeTriangle(const Point2& p0_, const Point2& p1_, const Point2& p2_,
                     bool bFill, double width);
