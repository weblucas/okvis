 ...
 */
CLASS_DECLSPEC
void getDirectedEdges(std::vector<Triangle2*>& vT,
 ...
 */
CLASS_DECLSPEC
void getUndirectedEdges(std::vector<Triangle2*>& vT,
...
bool fillHole(std::vector<std::pair<Segment2, Vector2> > vPolygonSegments,
              bool bWithRefine, bool bVerbose,
...
bool fillHole(std::vector<Point2>& vMeshCorners,
              std::vector<Segment2>& vPolygonSegments, bool bWithRefine,
              bool bVerbose, std::vector<Point2>& vCornersOut);
 ...
 */
CLASS_DECLSPEC
bool fillHole(Mesh3* pMesh, std::vector<Edge2>& vPolygonEdges, bool bWithRefine,
              bool bVerbose, std::vector<Point2>& vCornersOut);
#endif
 ...
 * \image latex edges-to-polygons.eps "Polygons created by edgesToPolygons"
 * width=12cm
 *
 */
CLASS_DECLSPEC
void edgesToPolygons(std::vector<Edge2>& vEdgesIn,
 ...
 */
CLASS_DECLSPEC
void getBorders(const std::vector<Triangle2*>& vT,
 ...
 *
 * This function returns the \e exact orientation of the points \p p0, \p p1, \p
 * p2 Possible values are \n ORIENTATION2_COLLINEAR if \p p0, \p p1, \p p2 are
 * located on a line, \n ORIENTATION2_CCW if \p p0, \p p1, \p p2 are
 * counterclockwise oriented \n ORIENTATION2_CW if \p p0, \p p1, \p p2 are
 * clockwise oriented \n
 *
...
FUNC_DECLSPEC
Orientation2 getOrientation2(const Point2* p0, const Point2* p1,
                             const Point2* p2);
...
FUNC_DECLSPEC
Orientation2 getOrientation2_mt(const Point2* p0, const Point2* p1,
                                const Point2* p2);
...
/// @private
FUNC_DECLSPEC
void setLic(const std::string& l1, const std::string& l2, const std::string& dt,
            const std::string& s1, const std::string& s2_);
/// @private
class Lic;
/// @private
Lic* getLic();
 ...
 */
FUNC_DECLSPEC
bool writePointsASCII(const char* filename,
