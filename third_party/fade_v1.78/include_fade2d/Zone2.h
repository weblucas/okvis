...
class CLASS_DECLSPEC Zone2 {
 public:
  ...
  /// @private
  Zone2(Dt2* pDt_, const std::vector<ConstraintGraph2*>& vConstraintGraphs_,
        ZoneLocation zoneLoc_, std::vector<Point2>& vStartPoints);

  /** \brief Get the zone location
   *
   * \returns ZL_INSIDE if the zone applies to the triangles inside one or more
   * ConstraintGraph2 objects@n ZL_OUTSIDE if the zone applies to the outside
   * triangles@n ZL_GLOBAL if the zone applies (dynamically) to all triangles@n
   * ZL_RESULT if the zone is the result of a set operation@n
   * ZL_GROW if the zone is specified by a set of constraint graphs and an inner
   * point@n \image html in_and_outside_zone.jpg "An ouside zone and in inside
   * zone" \image latex in_and_outside_zone.eps "An ouside zone and in inside
   * zone" width=12cm
   */
   ...
   * @param bShowFull specifies if only the zone or the full triangulation shall
   * be drawn
   * @param bWithConstraints specifies if constraint edges shall be drawn
   *
   */
  void show(const std::string& postscriptFilename, bool bShowFull,
   ...
   * @param bWithConstraints specifies if constraint edges shall be drawn
   */
  void show(Visualizer2* pVisualizer, bool bShowFull,
            bool bWithConstraints) const;
  ...
  */
   ...
   * @warning This method is fast but O(n), so don't call it frequently in a
   * loop.
   *
   */
   ...
   *
   * A Zone2 object might be defined by zero, one or more ConstraintGraph2
   * objects.
   */
 ...
 protected:
  ...
  /// @private
  void bfsFromWorkspace(std::vector<Triangle2*>& vWorkspace,
                        std::vector<Triangle2*>& vTriangles) const;
  /// @private
  Zone2* ctbz_treatCC(std::vector<Triangle2*>& vOneCC);
  /// @private
  void optimize(std::vector<Triangle2*>& vT,
                std::vector<Triangle2*>& vChangedT);
  // Data
  Dt2* pDt;
  ...
  CLASS_DECLSPEC
  friend Zone2* peelOffIf(Zone2* pZone, UserPredicateT* pPredicate,
                          bool bVerbose);
 ...
 * @return a new zone containing the union of the triangles of *pZone0 and
 * *pZone1
 ...
 * @return a new zone containing the triangles of *pZone0 minus the ones of
 * *pZone1
 * @note \p pZone0 and \p pZone1 must belong to the same Fade_2D object.
 */
CLASS_DECLSPEC
