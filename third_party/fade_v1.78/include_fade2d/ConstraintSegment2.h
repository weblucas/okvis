...
class ConstraintSegment2 {
 private:
  ...
  ConstraintSegment2(Point2* p0_, Point2* p1_,
 ...
 public:
  ...
  // Sets a specific ConstraintGraph2 as owner of the current
  // ConstraintSegment2.
  /// @private
  void addOwnerRecursive(ConstraintGraph2* pOwner);

  // Removes a specific ConstraintGraph2 as owner of the current
  // ConstraintSegment2.
  /// @private
   ...
   */
  void getChildrenRec(
      std::vector<ConstraintSegment2*>& vChildConstraintSegments);
  CLASS_DECLSPEC
  /** \brief Get the children and the split point
   * Retrieve the two direct children of the current ConstraintSegment2 as well
   * as the split point.
   */
  void getChildrenAndSplitPoint(ConstraintSegment2*& pCSeg0,
                                ConstraintSegment2*& pCSeg1,
                                Point2*& pSplitPoint);
  CLASS_DECLSPEC
  friend std::ostream& operator<<(std::ostream& stream,
                                  const ConstraintSegment2& cSeg);
